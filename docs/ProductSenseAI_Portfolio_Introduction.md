...
> 项目定位：AI Product Manager Portfolio MVP  
> 项目类型：可点击产品工作流原型 + 可选真实 AI Analysis 流程  
...
| 项目定位 | 将分散客户反馈转化为可审核的产品机会、PRD 草稿、实验方案和风险评估 |
| 目标用户 | AI Product Manager、Growth PM、Associate PM、需要处理大量反馈的早期产品团队 |
| 核心价值 | 帮助 PM 把“原始反馈”转化为“有证据、有优先级、有人审、可导出”的产品决策材料 |
| Live Demo | https://productsense-ai.vercel.app |
| GitHub | https://github.com/fy8618/productsense-ai |

ProductSense AI 是一个面向 AI 产品经理岗位的作品集 MVP。它不是生产级 SaaS，也不声称可以自动替代 PM 做路线图决策。项目重点展示的是：如何把 AI 能力嵌入真实产品发现流程，并通过 PM Review、证据追溯、RICE 调整、风险评估和 Markdown PRD 导出，让 AI 输出变得更可审核、更适合产品决策。

## 2. 项目背景

产品经理经常需要从客服工单、销售通话、用户访谈、取消问卷和使用反馈中判断“用户真正的问题是什么”。这些信息通常分散在不同渠道，格式不统一，且很难快速整理成可以用于产品评审的结构化材料。

传统人工整理流程存在几个问题：用户反馈分散、痛点归纳效率低、证据引用容易丢失、优先级判断缺少统一框架。引入 AI 后，PM 可以更快得到总结和建议，但也会出现新的风险：AI 输出可能缺少证据、夸大确定性，或把未经验证的假设写成事实。

因此，ProductSense AI 没有把 AI 设计成“自动拍板”的工具，而是设计成一个 AI Product Decision Agent：AI 负责整理、归纳、起草和提示风险，PM 负责审核证据、修正假设、调整优先级，并决定是否进入 prototype 阶段。

作品集演示还会受到现实限制影响，例如免费 API、provider availability、quota、billing 和 rate limit。因此项目保留了稳定的 Demo fallback workflow，确保即使无法使用 live AI，也能完整展示产品工作流。

## 3. 产品目标

ProductSense AI 的目标是用一个清晰、可运行、不过度工程化的 MVP 展示 AI PM 工作流设计能力：

- 从原始客户反馈中提取痛点、用户片段和频率信号。
- 将痛点转化为可评估的产品机会。
- 保留 Evidence Used，让每个建议可以追溯到用户反馈。
- 生成 PRD 草稿，但要求 PM 补充业务背景、假设和成功指标。
- 设计 Experiment Brief，帮助 PM 验证 hypothesis、metrics、guardrail 和 rollout plan。
- 使用 RICE 进行优先级评估，并允许 PM 调整 Reach、Impact、Confidence 和 Effort。
- 输出风险、开放问题、证据覆盖和谨慎的决策建议。
- 保留 human-in-the-loop，不自动做 roadmap、launch 或 investment decision。

## 4. 核心流程

ProductSense AI 的核心流程是：

**原始反馈输入 → AI Analysis → 痛点聚类 → 机会识别 → 证据追溯 → PM Review → AI PRD → Experiment Brief → RICE 调整 → AI Evaluation → Markdown Export → Demo fallback workflow**

1. 原始反馈输入  
   用户可以在 AI Analysis 页面粘贴客服工单、访谈记录、销售通话或问卷反馈，也可以通过 mock workflow 查看准备好的反馈样本。

2. AI Analysis  
   服务端 API 将反馈发送给配置的 OpenAI-compatible provider，并请求结构化 JSON 输出，包括痛点、证据、机会、RICE-style 字段、风险和人工审核问题。
...
5. 证据追溯  
   Evidence Used 模块展示每条 quote 支持了哪个 pain point 和 opportunity，帮助 PM 避免脱离用户证据的判断。

6. PM Review  
   AI 输出不会直接进入路线图。PM 需要查看 assumptions、open questions 和 human review warning，并选择一个机会继续。

7. AI PRD  
   被选中的机会进入 AI PRD 页面。页面基于结构化结果和 deterministic templates 生成 PRD 草稿，不再额外调用 AI。

8. Experiment Brief  
   PM 可以编辑 Hypothesis、Primary metric、Secondary metrics、Guardrail metric、Test design、Target segment、Success criteria、Rollout plan、Risks 和 Decision rule。
...
项目支持轻量级中文 / English UI toggle。语言选择保存在浏览器 localStorage 中，刷新后仍会保留。中文模式和英文模式都有对应的主要 UI 文案、mock data 和 demo fallback data。这个实现用于展示 localization awareness，不是生产级 i18n 框架。
...
ProductSense AI 把复杂的 PM 判断过程拆成多个可审核步骤：反馈输入、痛点聚类、机会识别、PRD 草稿、实验设计、RICE 调整、风险评估和导出。这比单一 chatbot 更接近真实产品工作流。
...
每个机会尽量保留 evidence quotes。用户可以看到 quote 支持的痛点和机会，从而降低 unsupported claim risk。
...
项目支持通过环境变量切换 OpenAI、Groq、Gemini 等 OpenAI-compatible providers，并在 provider failure、quota、billing 或 temporary availability 场景下提供 demo fallback，提升作品集演示稳定性。

### Bilingual product experience

项目支持中文和 English 两种 UI 模式，体现面向不同市场和受众的 localization awareness。
...
| Structured JSON validation | 保持 AI 输出形状稳定，便于前端渲染和错误处理 |
...
技术部分的重点不是展示复杂架构，而是说明 PM 对 AI 产品落地链路的理解：服务端保护 API key、结构化输出、错误兜底、数据本地保存和可解释的产品工作流。
...
- 设计 structured AI output，而不是依赖自由文本回答。
...
- 理解 server-side API route、环境变量、API key 安全和 provider configuration。
...
- Demo fallback 用于稳定作品集演示，不代表真实用户数据或真实模型评估结果。
...
- 进一步完善 production observability、错误监控、隐私和数据安全设计。
...
- Scoped a resilient portfolio MVP with mock workflow, demo fallback, and provider switching for OpenAI-compatible alternatives such as Gemini, Groq, and OpenAI.
